...
interface UseInstantEducationSearchReturn {
  hits: SearchHit[];
  loading: boolean;
  error: string | null;
...
/**
 * Custom hook for instant search with Meilisearch specifically for Education
 * Provides real-time search results with debouncing and highlighting
 */
export const useInstantEducationSearch = (
  query: string,
  debounceMs: number = 200,
  minQueryLength: number = 2
): UseInstantEducationSearchReturn => {
  ...
  const performSearch = useCallback(async (searchQuery: string) => {
    ...
    try {
      ...
      const params = new URLSearchParams({
        q: searchQuery,
        limit: '8',
        type: 'education'  // Filter for education only
      });
      ...
      console.error('Instant education search error:', err);
